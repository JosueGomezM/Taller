...
BEGIN
    ...
    DELETE FROM users WHERE email = 'equiros@multiecocr.com';
    
    -- Luego eliminar de auth.users
    DELETE FROM auth.users WHERE email = 'equiros@multiecocr.com';
    ...
    IF NOT EXISTS (
        SELECT 1 FROM auth.users WHERE email = 'equiros@multiecocr.com'
    ) AND NOT EXISTS (
        SELECT 1 FROM users WHERE email = 'equiros@multiecocr.com'
    ) THEN
        ...
        BEGIN
            ...
            VALUES (
                ...
                'equiros@multiecocr.com',
                crypt('pucho2025', gen_salt('bf')),
                now(),
                '{"provider":"email","providers":["email"]}',
                '{"full_name":"Erick Quiros"}',
                now(),
                now(),
                '',
                '',
